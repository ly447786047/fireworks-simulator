...
// 随机文字烟花内容
const randomWords = ["新年快乐", "马到成功", "2026", "大吉大利", "身体健康", "万事如意", "平安喜乐", "万事顺遂", "爱你老己", "岁岁平安", "健康快乐", "前程似锦", "笑口常开", "快乐加倍", "被爱", "好运常在", "幸运", "马上有钱", "健康", "可爱", "暴富", "好运", "幸福", "顺遂", "开心", "快乐"];
// const randomWords = ["新年快乐", "胡秀银"];
const wordDotsMap = {};
randomWords.forEach((word) => {
	wordDotsMap[word] = MyMath.literalLattice(word, 3, "华文琥珀", "120px");
});

// 空的setLoadingStatus函数（避免报错）
...
function createStartButton() {
	...
	if (document.getElementById('start-back')) return;
	...
	btn.addEventListener('mouseenter', () => {
		btn.style.boxShadow = '0 8px 32px 0 rgba(31, 38, 135, 0.3), 0 0 15px rgba(100, 149, 237, 0.4)';
		btn.style.transform = 'translate(-50%, -50%) scale(1.1)';
		// 新增：hover时边框变亮、变宽，带发光感
		btn.style.border = '1px solid rgba(255, 255, 255, 0.8)';
		btn.style.boxShadow = '0 8px 32px 0 rgba(31, 38, 135, 0.3), 0 0 20px rgba(255, 255, 255, 0.5)';
	});
	btn.addEventListener('mouseleave', () => {
		btn.style.boxShadow = '0 8px 32px 0 rgba(31, 38, 135, 0.15)';
...
const floralShell = (size = 1) => ({
	...
	color: Math.random() < 0.65 ? "随机" : Math.random() < 0.15 ? randomColor() : [randomColor(), randomColor({ notSame: true })],
	floral: true,
});
...
function randomShellName() {
	return Math.random() < 0.5 ? "菊花形" : shellNames[(Math.random() * (shellNames.length - 1) + 1) | 0];
}
...
//还有，这不创建烟花，只返回工厂函数。
const fastShellBlacklist = ["落叶", "繁花", "垂柳"];
...
function startSequence() {
	...
	if (finaleSelector()) {
		...
		if (currentFinaleCount < finaleCount) {
			...
			// console.log(`发射第 ${currentFinaleCount} 发烟花`);

			return randomDelay; // 返回实际使用的随机延迟值，而非固定50
		} else {
			currentFinaleCount = 0; // 重置计数，准备下次终章
			// console.log("终章结束，正在等待下一组烟花");
			return 1000; // 所有烟花发射完后返回1秒间隔
		}
	}
...
function getWordDots(word) {
	...
	var res = MyMath.literalLattice(word, 3, "华文宋体", fontSize + "px");
	return res;
}
...
const soundManager = {
	...
	// 初始化音频上下文（解决移动端需要用户交互才能初始化的问题）
	initAudioContext() {
		if (!this.ctx) {
			try {
				this.ctx = new (window.AudioContext)();
				// setLoadingStatus("音频引擎已就绪");
			} catch (e) {
				// console.error("初始化音频上下文失败:", e);
				// setLoadingStatus("音频加载失败（浏览器不支持）");
				return false;
			}
		}
		return true;
	...
	preload() {
		...
		if (!this.initAudioContext()) {
			return Promise.reject("音频上下文初始化失败");
		}

		// setLoadingStatus("正在加载音效资源...");
		...
		function checkStatus(response) {
			...
			const customError = new Error(`加载音频失败: ${response.statusText}`);
		...
		types.forEach((type) => {
					...
					.then((data) => {
						return new Promise((resolve, reject) => {
							this.ctx.decodeAudioData(
								...
								(error) => reject(`解码音频失败 ${fileName}: ${error}`)
							);
						});
					})
					.catch((error) => {
						// console.error(`加载音频 ${fileName} 失败:`, error);
						// setLoadingStatus(`音效 ${fileName} 加载失败（将静音播放）`);
						// 即使单个音频失败，也不中断整体加载
						return null;
					});
			...
			Promise.all(filePromises).then((buffers) => {
				...
				if (source.buffers.length === 0) {
					// console.warn(`类型 ${type} 的所有音频都加载失败`);
				}
			});
		});
	...
	resumeAll() {
		...
		if (this.ctx && this.ctx.state === "suspended") {
			...
			setTimeout(() => {
				...
				}).catch(e => console.error("恢复音频失败:", e));
			}, 100);
		}
	},
	...
	/**
	 * 播放指定类型的音效
	 * @param {string} type - 音效类型（lift/burst/burstSmall/crackle/crackleSmall）
	 * @param {number} scale - 音量缩放（0-1，0为静音）
	 */
	playSound(type, scale = 1) {
		// 校验参数
		scale = Math.max(0, Math.min(1, scale)); // 限制在0-1之间
		if (!canPlaySoundSelector() || simSpeed < 0.95 || !this.ctx) {
			return;
		}
